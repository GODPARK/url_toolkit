  ...
  <v-app style="min-width:750px;">
    ...
    >
      ...
      <v-btn-toggle
        ...
        group
      >
        <v-btn small @click="clickInfoButton()">
          <v-icon small>mdi-information</v-icon>
          info
        </v-btn>
        <v-btn small @click="clickEditButton()">
    ...
    <v-main>
      <v-container>
        <v-row>
          ...
          <v-col cols="12" v-if="!editBool">
            <main-info ref="mainInfoRef" :pageInfo="pageInfo" />
          </v-col>
          <v-col cols="12"
            v-if="editBool && pageInfo.search !== undefined && pageInfo.search !== ''">
            <url-edit ref="urlEditRef" :pageInfo="pageInfo" :testBool="testBool"/>
          </v-col>
          <v-col
            cols="12"
            v-if="!editBool && pageInfo.search !== undefined && pageInfo.search !== ''"
          >
            <params-info ref="paramsInfoRef" :pageInfo="pageInfo" :testBool="testBool"/>
          </v-col>
        </v-row>
      </v-container>
      <v-footer padless>
        <v-col
          ...
          cols="12"
        >
          Chrome Extension <strong>Urltoolkit</strong> @2022
        </v-col>
      </v-footer>
    </v-main>
  </v-app>
...
import UrlEdit from './components/UrlEdit.vue';
...
export default {
  ...
  methods: {
    async getCurrentUrlInfo() {
      ...
      } else if (this.appMode === 'cdn') {
        this.cdnBool = true
        const urlToolkitUrl = new URL(window.location.href)
        const checkStr ="?uturl="

        if (urlToolkitUrl.search !== undefined && urlToolkitUrl.search !== '') {
          if (urlToolkitUrl.search.indexOf(checkStr) !== -1) {
            this.pageInfo = new URL(urlToolkitUrl.search.replace("?uturl=", ''))
          } else {
            this.pageInfo = urlToolkitUrl;
          }
        }
      } else {
        this.testBool = true;
